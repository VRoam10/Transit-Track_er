...
router.get("/", async (req: Request, res: Response) => {
    try {
        const connector = await prisma.connector.findUnique({
            ...
            include: { nextPassage: true },
        });

        if (!connector?.nextPassage) {
            return res.status(404).json({ error: "Next passage config not found for this connector" });
        }

        const { apiUrl, transformation, params } = connector.nextPassage;
        ...
        const url = buildUrl(apiUrl, params, req.query as Record<string, any>);
