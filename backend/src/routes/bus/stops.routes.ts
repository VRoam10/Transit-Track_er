...
router.get("/:idLigne/:sens", async (req: Request<{ idLigne: string, sens: string }, any, any, { pages?: string }>, res: Response) => {
  try {
    ...
    getDirection(req.params.idLigne).then((directionData) => {
      ...
      axios.get(`https://data.explore.star.fr/api/explore/v2.1/catalog/datasets/tco-bus-topologie-dessertes-td/records?order_by=ordre&select=idligne%2Cnomcourtligne%2Cidarret%2Cnomarret%2Cordre&limit=50&refine=idparcours%3A%22${direction.parcoursId}%22&offset=${offset}`).then((response) => {
...
function getDirection(idLigne: string) {
  return axios.get(`https://data.explore.star.fr/api/explore/v2.1/catalog/datasets/tco-bus-topologie-parcours-td/records?select=sens%2Cnomarretarrivee%2Cid&where=idligne%3D%22${idLigne}%22&limit=20&refine=type%3A%22Principal%22`).then((response) => {
