...
router.get("/:idStop", async (req, res) => {
  try {
    axios.get(`https://data.explore.star.fr/api/explore/v2.1/catalog/datasets/tco-metro-circulation-deux-prochains-passages-tr/records?limit=20&select=idjdd%2Cidligne%2Cnomarret%2Csens%2Carriveefirsttrain%2Ccoordonnees%2Cheureextraction&refine=idjdd%3A${req.params.idStop}`).then((response) => {
